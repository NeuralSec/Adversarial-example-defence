...
from setup_cifar import CIFAR, CIFAR2
import os
...
def train_oracle(data, file_name, params, num_epochs=50, batch_size=128, train_temp=1, init=None):
    """
    Oracle neural network training procedure.
    """
    model = Sequential()
    print(data.train_data.shape)
    ...
    model.add(Conv2D(params[4], (3, 3), padding='same'))
    model.add(Activation('relu'))
    model.add(Conv2D(params[5], (3, 3), padding='same'))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Dropout(0.2))
...
train_oracle(CIFAR2(), "models/cifar_oracle", [32, 32, 64, 64, 128, 128, 512], num_epochs=50)
#train_oracle(MNIST(), "models/mnist_oracle", [32, 32, 64, 64, 128, 128, 512], num_epochs=50)
